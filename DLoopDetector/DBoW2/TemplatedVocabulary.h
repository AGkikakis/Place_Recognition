...
public:
  ...
  /**
   * Overloaded stopwords function
   * input stopwords represent a percentage of the overall number of words
   * we wish to stop. Function makes a copy of the vector containing all the
   * words and sorts it. The weight of the word located in the index where the
   * stopwords % is located is retrieved and passed to the built in function.
   */
  int stopWords(int stopwords);
...
protected:
  ...
  /**
   * Function used to compare two words by their weight
   * @return true if the weight of word n1 is smaller than that of n2
   */
...
// Takes as input two nodes an returns true if node1.weight < node2.weight
template<class TDescriptor, class F>
bool TemplatedVocabulary<TDescriptor,F>::cmp( const Node *n1, const Node *n2)
{
  return ( n1->weight < n2->weight );
}

template<class TDescriptor, class F>
int TemplatedVocabulary<TDescriptor,F>::stopWords(int stopwords)
{
  //copy leaf vector
  std::vector<Node*> sorted;
  sorted = this->m_words;
  //sort nodes
  sort( sorted.begin(), sorted.end(), TemplatedVocabulary<TDescriptor,F>::cmp);
  //get the value of the weight located at stopwords%
  double minWeight = sorted[ (int)( (float(stopwords)/100.0f) * m_words.size()) ]->weight;
  //make all weights lower than minweight zero
  return stopWords(minWeight);
}
