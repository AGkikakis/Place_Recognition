...
public:
  ...
  /**
  * Function that stores the database in a txt file along with two seperate
  * files containing all the descriptors of all images and and their keypoints
  * @param filename
  */
  void saveDatabase(const string &filename,const string &descFilename,
    const string &keyFilename);

  /**
  * Loads an already existing database from filename file
  * The contents of the database and the detector
  * @param filename name of the database
  * @param keyFilename name of file containing the keys
  * @param descFilename name of file containing the descriptors
  */
  void loadDatabase(const string &filename, const string &keyFilename,
    const string &descFilename);

  /**
  * Function that stores all descriptors and keypoints in two seperate files
  * for geometrical verification when the database is loaded
  */
  void saveFeatures( const vector<vector<cv::KeyPoint> > &keys,
    const vector<vector<TDescriptor> > &descriptors, const string &keyFilename,
    const string &descFilename);

  /**
  * Function that loads all the descriptors and keypoints assosiated with
  * every image in the database
  */
  void loadFeatures( const string &keyFilename, const string &descFilename);
  ...
  /*
   * Changes dislocal value of Parameters structure
   */
  inline void setDislocal(const int &dis)
  {
    m_params.dislocal = dis;
  }
...
template<class TDescriptor, class F>
void TemplatedLoopDetector<TDescriptor, F>::saveDatabase(const string &filename,
  const string &keyFilename, const string &descFilename )
{
  m_database->save(filename);
  saveFeatures( m_image_keys, m_image_descriptors, keyFilename, descFilename);
}

// --------------------------------------------------------------------------

template<class TDescriptor, class F>
void TemplatedLoopDetector<TDescriptor, F>::loadDatabase(const string &filename,
  const string &keyFilename, const string &descFilename)
{
  m_database->load(filename);
  loadFeatures(keyFilename, descFilename);
}

// --------------------------------------------------------------------------

template<class TDescriptor, class F>
void TemplatedLoopDetector<TDescriptor, F>::saveFeatures(
  const vector<vector<cv::KeyPoint> > &keys,
  const vector<vector<TDescriptor> > &descriptors ,const string &keyFilename,
  const string &descFilename)
{
  ofstream file_desc(descFilename.c_str());
  cv::FileStorage fs(keyFilename, cv::FileStorage::WRITE);
  
  for (unsigned int frame = 0; frame < descriptors.size(); ++frame)
  {
    vector<TDescriptor> frame_descriptors = descriptors[frame];
    
    for (unsigned int fr_desc=0; fr_desc<frame_descriptors.size(); ++fr_desc)
    {
      TDescriptor descriptor = frame_descriptors[fr_desc];
      for (unsigned int disc = 0; disc < descriptor.size(); ++disc)
        file_desc<<descriptor[disc]<<',';
      file_desc<<endl;
    }
    file_desc<<endl;
    
    vector<cv::KeyPoint> keypoints = keys[frame];
    stringstream s;
    s<<"keypoints_"<<frame;
    cv::write(fs, s.str() , keypoints);
  }
  file_desc.close();
  fs.release();
}
...
template<class TDescriptor, class F>
void TemplatedLoopDetector<TDescriptor, F>::loadFeatures( const string &keyFilename,
  const string &descFilename )
{
  //////////////////// Load descriptors ///////////////////////////////////////
  std::ifstream file(descFilename.c_str());
  std::string line;
  int count = 0;
  // vector that will hold all the descriptors of a frame
  vector<TDescriptor> frame;
  //frame.reserve(F::dimensions());
  while ( std::getline(file, line) )
  {
    if ( !line.empty() )
    {
      ...
      while (ss)
      {
        ...
        desc.push_back(atof(s.c_str()));
      }//end while
      frame.push_back(desc);
    }//end if
    else// descriptors of new image
    {
      m_image_descriptors.push_back(frame);
      frame.clear();
    }
  }//end while
  /////////////////// Load keypoints //////////////////////////////////////////
  // Using built in function opencv to stora and load keyframes
  cv::FileStorage fs2(keyFilename, cv::FileStorage::READ);
  for (unsigned int key = 0; key < m_database->size(); ++key)
  {
    vector<cv::KeyPoint> k;
    stringstream node;
    node<<"keypoints_"<<key;
    cv::FileNode keyNode = fs2[node.str()];
    cv::read( keyNode, k);
    m_image_keys.push_back(k);
  }
  fs2.release();
}
